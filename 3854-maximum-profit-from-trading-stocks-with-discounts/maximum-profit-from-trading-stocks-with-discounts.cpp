...
public:
    ...
    // Merge: cur[b] + childDp[x] => newCur[b]
    ...
    myPair dfs(int u, vector<vector<int>>& child, vector<int>& present, vector<int>& future, int budget) {
        // Profit from children only:
        // - if u NOT bought: children see "parent not bought" => use child.dp0
        // - if u bought:     children see "parent bought"     => use child.dp1
